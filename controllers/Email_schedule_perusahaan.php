...
class Email_schedule_perusahaan extends AdminController
{
    ...
    {
        if (!staff_can('create', 'perusahaans')) {
            ajax_access_denied();
        }

        $this->load->model('perusahaans_model');

        if ($this->input->post()) {
            $data = $this->input->post();

            $this->email_schedule_model->create($id, 'perusahaan', [
                'scheduled_at' => to_sql_date($data['scheduled_at'], true),
                'cc'           => $data['cc'],
                'contacts'     => $data['sent_to'],
                'attach_pdf'   => isset($data['attach_pdf']) ? 1 : 0,
                'template'     => 'perusahaan_send_to_customer',
            ]);

            set_alert('success', _l('email_scheduled_successfully'));
            redirect(admin_url('perusahaans/list_perusahaans/' . $id));
        }

        $data            = $this->scheduleData($id);
        $data['formUrl'] = admin_url('email_schedule_perusahaan/create/' . $id);
        $data['date']    = get_scheduled_email_default_date();
        $this->load->view('admin/perusahaans/schedule', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if (staff_can('edit', 'perusahaans') || $data['perusahaan']->addedfrom == get_staff_user_id()) {
            if ($this->input->post()) {
                $data = $this->input->post();
                ...
                set_alert('success', _l('email_scheduled_successfully'));
                redirect(admin_url('perusahaans/list_perusahaans/' . $schedule->rel_id));
            }

            $data['schedule'] = $schedule;
            $data['formUrl']  = admin_url('email_schedule_perusahaan/edit/' . $id);
    ...
    {
        $perusahaan = $this->perusahaans_model->get($id);

        $data['perusahaan'] = $perusahaan;

        $this->load->model('emails_model');
        $template_name = 'perusahaan_send_to_customer';
