...
<div class="row">
<?php if(count($perusahaan_years) > 1 || isset($currencies)){ ?>
  <div class="col-md-12 simple-bootstrap-select mbot5">
   <?php if(isset($currencies)){ ?>
    <select class="selectpicker" data-width="auto" name="total_currency" onchange="init_perusahaan_total();">
    ...
    <?php } ?>
      <?php
      if(count($perusahaan_years) > 1){ ?>
      <select data-none-selected-text="<?php echo date('Y'); ?>" data-width="auto" class="selectpicker" multiple name="perusahaan_total_years" onchange="init_perusahaan_total();">
         <?php foreach($perusahaan_years as $year){ ?>
  ...
  foreach($totals as $key => $data){
    $class = equipment_status_color_class($data['status']);
    $name = equipment_status_by_id($data['status']);
    ?>
